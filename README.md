...
# 3. 索引项目
python -m src.cli.main index ./your-project/ --force

# 4. 开始对话
python -m src.cli.main chat --workspace ./your-project/
```
...
```bash
# ── 索引 ──────────────────────────────────────────
python -m src.cli.main index <目标目录>               # 增量索引
python -m src.cli.main index ./my-project/ --force    # 强制重建
python -m src.cli.main index ./my-project/ -c config.yaml  # 指定配置

# ── 单次问答 ─────────────────────────────────────
python -m src.cli.main ask "怎么安装？" -w ./my-project/
python -m src.cli.main ask "入口文件是哪个？" -w ./my-project/

# ── 交互对话（推荐） ─────────────────────────────
python -m src.cli.main chat -w ./my-project/
#   输入 /reset 清空历史   /quit 退出

# ── 测试 ──────────────────────────────────────────
pytest tests/ -v                          # 全部测试
pytest tests/ -v -m "not slow"            # 跳过下载模型的慢速测试
```
...
| 命令 | 参数 | 说明 |
|:---|:---|:---|
| `index` | `<目录>` | **必填**，要索引的目标项目路径 |
| | `--force, -f` | 强制重建索引（清空已有数据） |
| | `--config, -c` | 配置文件路径，默认 `config.yaml` |
| `ask` | `<问题>` | **必填**，要问的问题 |
| | `--workspace, -w` | Agent 工具的文件访问范围（默认当前目录） |
| | `--config, -c` | 配置文件路径 |
| `chat` | `--workspace, -w` | 同上，Agent 工具可访问的文件范围 |
| | `--config, -c` | 配置文件路径 |

> **注意**：`--workspace` *应指向你索引过的同一目录*，这样混合检索结果和工具操作都在同一个项目里。
